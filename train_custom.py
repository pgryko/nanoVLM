...
def get_dataloaders(train_cfg, vlm_cfg):
    ...
    train_dataset, val_dataset = random_split(
        full_dataset, [train_size, val_size], generator=torch.Generator().manual_seed(0)
    )
...
def train(train_cfg, vlm_cfg):
    ...
    if is_master():
        ...
        print(
            f"Training summary: {len(train_loader.dataset)} samples, {len(train_loader)} batches/epoch"
        )
        print(
            f"Validation summary: {len(val_loader.dataset)} samples, {len(val_loader)} batches/epoch"
        )
    ...
    param_groups = [
        {
            "params": list(
                model.module.MP.parameters() if is_dist() else model.MP.parameters()
            ),
            "lr": train_cfg.lr_mp,
        },
        {
            ...
            + list(
                model.module.vision_encoder.parameters()
                if is_dist()
    ...
    while global_step < train_cfg.max_training_steps:
        ...
        for i, batch in enumerate(train_loader):
            ...
            if is_update_step:
                ...
                adj_lr_backbones = get_lr(
                    global_step, train_cfg.lr_backbones, train_cfg.max_training_steps
                )
            ...
            if (
                ...
                with torch.no_grad():
                    ...
                    if is_master():
                        print(
                            f"Step: {global_step}, Train Loss: {batch_loss:.4f}, Val Loss: {avg_val_loss:.4f}"
                        )
        ...
        if is_master():
            print(
                f"Epoch: {epoch}, Step: {global_step}/{train_cfg.max_training_steps}, "
...
def main():
    ...
    # Logging arguments
    parser.add_argument(
        "--log_wandb", action="store_true", help="Log to Weights & Biases"
    )
    parser.add_argument(
    ...
    if is_master():
        ...
        print(
            f"Effective batch size: {train_cfg.batch_size * train_cfg.gradient_accumulation_steps * get_world_size()}"
        )
