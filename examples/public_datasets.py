...
def convert_coco_captions(output_path: str, split: str = "train", limit: int = None):
    ...
    for item in tqdm(dataset):
        ...
        caption = (
            item["sentences"]["raw"][0]
            if item["sentences"]["raw"]
...
def convert_vqav2(output_path: str, limit: int = None):
    ...
    for item in tqdm(dataset):
        ...
        if item["answers"]:
            answer = max(
                set(item["answers"]["answer"]), key=item["answers"]["answer"].count
            )
        else:
            continue
...
def convert_vizwiz(output_path: str, split: str = "train", limit: int = None):
    ...
    for item in tqdm(dataset):
        ...
        if not item["answers"] or not any(
            ans["answer_confidence"] == "yes" for ans in item["answers"]
        ):
            continue
...
def download_images_from_urls(
    dataset_path: str, image_dir: str, url_key: str = "image_path"
):
    """
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert public datasets to NanoVLM format"
    )
    parser.add_argument(
        "--dataset",
        choices=["llava", "coco", "vqav2", "vizwiz", "scienceqa"],
        required=True,
