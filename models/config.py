...
class VLMConfig:
    ...
    extra_token_amount: int = (
        1  # Number of extra tokens for the VLM (image start, image end, image token)
    )
    lm_vocab_size: int = (
...
class TrainConfig:
    ...
    lmms_eval_tasks: str = (
        "mmstar,mmmu,ocrbench,textvqa"  # Pass additional task as one string, seperated by commas without spaces (e.g. 'mmstar,mmmu,ocrbench')
    )
