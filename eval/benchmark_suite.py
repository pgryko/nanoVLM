...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--lm_tokenizer",
        type=str,
        default="HuggingFaceTB/cosmo2-tokenizer",
        help="LLM tokenizer identifier.",
    )
    parser.add_argument(
        "--mp_pixel_shuffle_factors",
        type=int,
        nargs="+",
        default=[1, 2, 4],
    ...
    parser.add_argument(
        "--num_runs", type=int, default=10, help="Number of times to run the benchmark."
    )
    parser.add_argument(
        "--warmup_runs",
        type=int,
        ...
        with open(results_file, "r") as f:
            ...
            for r in existing_results_list:
                ...
                if all(
                    ...
                    key = (
                        r["vit_model_type"],
                        r["mp_pixel_shuffle_factor"],
                        r["lm_model_type"],
                    )
                    cached_results[key] = r
                else:
                    print(
                        f"Warning: Skipping invalid or incomplete entry in '{results_file}': {r}"
                    )
            print(
                f"Loaded {len(cached_results)} existing valid results from '{results_file}'."
            )
    except FileNotFoundError:
        print(f"'{results_file}' not found. Starting with an empty cache.")
    except json.JSONDecodeError:
        print(
            f"Error decoding JSON from '{results_file}'. Starting with an empty cache."
        )

    # Generate combinations
    all_combinations = list(
        itertools.product(
            args.vit_model_types, args.mp_pixel_shuffle_factors, args.lm_model_types
        )
    )
