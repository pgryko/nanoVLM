...
class TestLanguageModel(unittest.TestCase):
    ...
    def test_kv_caching_consistency(self):
        ...
        self.assertTrue(
            torch.allclose(
                logits_no_cache_last_token, logits_with_cache_last_token, atol=1e-5
            ),
        ...
        for i in range(seq_len):
            ...
            output_step, kv_cache_step = self.model(
                current_input, kv_cache=kv_cache_step, start_pos=i
            )
            # output_step is [B, 1, vocab_size]
            output_tokens_with_cache_list.append(output_step)

        # Concatenate all single token outputs
        output_with_cache_full = torch.cat(
            output_tokens_with_cache_list, dim=1
