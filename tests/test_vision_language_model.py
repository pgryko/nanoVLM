...
class TestVisionLanguageModel(unittest.TestCase):
    def setUp(self):
        ...
        self.model = VisionLanguageModel(
            self.cfg, load_backbone=False
        )  # Don't load pretrained for unit test
    ...
    def test_generate_kv_caching_consistency(self):
        ...
        image_input = torch.randn(
            batch_size, 3, self.cfg.vit_img_size, self.cfg.vit_img_size
        )
        # Dummy prompt input_ids
        prompt_ids = torch.randint(
            0, self.cfg.lm_vocab_size, (batch_size, prompt_seq_len)
        )
